...
namespace ArticleCancer.Infrastructure.Services.Concrete
{
    public class DashboardService : IDashboardService
    {
        ...
        public async Task<List<int>> GetYearlyArticleByUserCounts()
        {
            var userID = _user.GetLoggedInUserId();
            var articles = await _unıtOfWork.GetRepository<Article>().GetAllAsync(x => !x.IsDeleted && x.UserID == userID);
        ...
        public async Task<int> GetTotalArticleCountByUser()
        {
            var userID = _user.GetLoggedInUserId();
            var articleCount = await _unıtOfWork.GetRepository<Article>().CountAsync(x => x.UserID == userID);
            return articleCount;
        }

        public Task<int> GetTotalCategoryCountByUser()
        {
        ...
        public async Task<int> GetTotalNewCountByUser()
        {
            var userID = _user.GetLoggedInUserId();
            var articleCount = await _unıtOfWork.GetRepository<New>().CountAsync(x => x.UserID == userID);
            return articleCount;
        }
    }
}
